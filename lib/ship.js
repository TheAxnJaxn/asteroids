...
(function () {
  ...
  var Ship = Asteroids.Ship = function(attrs) {
    attrs['color'] = attrs['color'] || "#4A91AF";
  ...
  Ship.prototype.speedUp = function speedUp () {
    this.accelerate(1);
  };

  Ship.prototype.slowDown = function slowDown () {
    this.accelerate(-1);
  };
  ...
  Ship.prototype.accelerate = function accelerate (num) {
    ...
    if (newSpeed > Ship.MAX_SPEED) {
      this.vel[0] = this.vel[0] * Ship.MAX_SPEED / newSpeed;
      this.vel[1] = this.vel[1] * Ship.MAX_SPEED / newSpeed;
    }
  };
