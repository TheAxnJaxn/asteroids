...
(function () {
  ...
  Ship.prototype.accelerate = function accelerate () {
    if (this.getSpeed() > 10)
      return;
    this.vel[0] += 1 * Math.sin(this.direction);
    this.vel[1] += 1 * Math.cos(this.direction);
  };

  Ship.prototype.decelerate = function decelerate () {
    if (this.getSpeed() < -10)
      return;
    this.vel[0] -= 1 * Math.sin(this.direction);
    this.vel[1] -= 1 * Math.cos(this.direction);
